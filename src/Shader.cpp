...
void Shader::ChangeShaders(const std::string& vertexShaderSource, const std::string& fragmentShaderSource){
    ...
    // Link our program
    // These have been compiled already.
    glAttachShader(m_shaderID,myVertexShader);
    glAttachShader(m_shaderID,myFragmentShader);
    // Link our programs that have been 'attached'
    glLinkProgram(m_shaderID);
    glValidateProgram(m_shaderID);

    // Once the shaders have been linked in, we can delete them.
    glDetachShader(m_shaderID,myVertexShader);
    glDetachShader(m_shaderID,myFragmentShader);

    glDeleteShader(myVertexShader);
    glDeleteShader(myFragmentShader);

    if(!CheckLinkStatus(m_shaderID)){
        Log("CreateShader","ERROR, shader did not link! Were there compile errors in the shader?");
    }
}
