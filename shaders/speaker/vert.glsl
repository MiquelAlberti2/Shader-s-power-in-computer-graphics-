...
void main()
{
    ...
    //our grid is 512x512, so we subtract 206 to "center" our cordinates
    float x = position.x - 206;
    float z = position.z - 206;

    float A = 5;

    new_position.y = A*sin(sqrt(x*x + z*z)/10) + 10*sin(20*time)*(20/(sqrt(x*x + z*z)+5));

    if(position.y != 0){
        height = (new_position.y+5)/5;
    }
    else{
        height = 0;
    }
