...
void main()
{
    ...
    //for now we wil ignore the texture, and set the color to blue
    //diffuseColor = texture(u_DiffuseMap, v_texCoord).rgb;

    float red, green, blue;

    // I want to obtain:
    // color of the sea (0, 0.4118, 0.5804) where height is small
    // color of the sand (0.761,0.698,0.502) where height is big
    // in the position where height is small,
    // I want to obtain the sea color: 
    red =   height*0        + (1-height)*0.761;
    green = height*0.4118 + (1-height)*0.698;
    blue =  height*0.5804  + (1-height)*0.502;
