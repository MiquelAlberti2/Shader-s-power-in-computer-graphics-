...
void main()
{
    ...
    float x = position.x;
    float z = position.z;
    float k = 0.2; //control the size of the wave length 
    float w = 1; //controls the oscillation frequency
    float A = 5; //controls the amplitud
    new_position.y = A*cos(k*x - w*time) + A*sin(k*z - w*time);

    if(position.y != 0){
        height = (new_position.y+1)/5;
    }
    else{
        height = 0;
    }
